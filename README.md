...
2. **Road-requiring buildings** (Great Buildings, event buildings) are placed near the Town Hall using a greedy BFS (Breadth-First Search) pass — scanning outward from the Town Hall so buildings cluster close to it, minimizing road length. Building set partners (Road to Victory, Iridescent Garden, Serpent accessories) are reserved adjacent to their anchors before roads are routed.
